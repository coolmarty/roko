...
BeelineMovement::Beeline(Point3 *dest){
	destination = dest;
}


void BeelineMovement::MovePath(Point3 *position, Direction *direction, Vector3 *velocity){
	// Vector from destination to position
	Vector3 distanceVector = new Vector(destination.GetX() - position.GetX(), 
